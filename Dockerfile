...
RUN apt install -y wget python3 pkg-config build-essential zlib1g-dev pkg-config libglib2.0-dev binutils-dev libboost-all-dev autoconf libtool libssl-dev libpixman-1-dev libpython-dev python-pip python-capstone virtualenv

RUN cd riscv-gnu-toolchain &&  ./configure --prefix=/usr/local && make install && make linux
